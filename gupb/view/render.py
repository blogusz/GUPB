...
class SpriteRepository:
    def __init__(self) -> None:
        ...
        self.sprites: dict[Any, Sprite] = {
            ...
            characters.Tabard.ORANGE: load_sprite(
                "characters", "champion_orange", BLACK
            ),
            ...
            characters.Tabard.STRIPPED: load_sprite(
                "characters", "champion_stripped", BLACK
            ),
            characters.Tabard.TURQUOISE: load_sprite(
                "characters", "champion_turquoise", BLACK
            ),
            characters.Tabard.VIOLET: load_sprite(
                "characters", "champion_violet", BLACK
            ),
            characters.Tabard.WHITE: load_sprite("characters", "champion_white", BLACK),
            characters.Tabard.YELLOW: load_sprite(
                "characters", "champion_yellow", BLACK
            ),
            characters.Tabard.ANCYMON: load_sprite(
                "characters", "champion_ancymon", BLACK
            ),
        ...
        self.champion_sprites: dict[
            tuple[characters.Tabard, characters.Facing], Sprite
        ] = {
            (tabard, facing): pygame.transform.rotate(
                self.sprites[tabard], self.rotation_values[facing]
            )
    ...
    def scale_sprites(
        self, window_size: Tuple[int, int], arena_size: Tuple[int, int]
    ) -> Tuple[int, int]:
        self.size = (
            int(window_size[0] / arena_size[0]),
            int(window_size[1] / arena_size[1]),
        )
        ...
        for sprite in self.champion_sprites:
            self.champion_sprites[sprite] = self.scale_sprite(
                self._champion_sprites[sprite], self.size
            )
...
class Renderer:
        ...
        while not game.finished:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.VIDEORESIZE:
                    new_size = self.sprite_repository.scale_sprites(
                        (event.w, event.h), game.arena.size
                    )
    ...
    def _resize_window(self, game: games.Game) -> pygame.Surface:
        ...
        window_size = (
            self.sprite_repository.size[0] * arena_x_size,
            self.sprite_repository.size[1] * arena_y_size,
        )
    ...
    def _render_starting_screen(self):
        ...
        while wait_for_start_key:
            GAME_FONT.render_to(
                self.screen, (70, 180), "Press X to start..!", (250, 250, 250)
            )
            pygame.display.flip()
    ...
    def _render(
        self, game: games.Game, show_sight: Optional[characters.Champion]
    ) -> None:
    ...
    def _render_arena(self, game: games.Game, background: pygame.Surface) -> None:
        def render_character() -> None:
            ...
            pygame.draw.rect(
                ...
                prepare_heath_bar_rect(
                    min(characters.CHAMPION_STARTING_HP, tile.character.health)
                ),
            )

        for i, j in game.arena.terrain:
            blit_destination = (
                i * self.sprite_repository.size[0],
                j * self.sprite_repository.size[1],
            )
    ...
    def _render_sight(
        self,
        game: games.Game,
        show_sight: characters.Champion,
