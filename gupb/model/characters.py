...
class Champion:
    def __init__(
        self, starting_position: coordinates.Coords, arena: arenas.Arena
    ) -> None:
    ...
    def description(self) -> ChampionDescription:
        return ChampionDescription(
            self.controller.name, self.health, self.weapon.description(), self.facing
        )
    ...
    def act(self) -> None:
        if self.alive:
            ...
            verbose_logger.debug(
                f"Champion {self.verbose_name()} picked action {action}."
            )
            ChampionPickedActionReport(self.verbose_name(), action.name).log(
                logging.DEBUG
            )
            action(self)
    ...
    def assess_idle_penalty(self) -> None:
        if (
            ...
            and self.previous_facing == self.facing
        ):
            self.time_idle += 1
        else:
            self.time_idle = 0
        if self.time_idle >= PENALISED_IDLE_TIME:
            verbose_logger.debug(
                f"Champion {self.verbose_name()} penalised for idle time."
            )
    ...
    def pick_action(self) -> Action:
        if self.controller:
            ...
            knowledge = ChampionKnowledge(
                self.position, self.arena.no_of_champions_alive, visible_tiles
            )
            try:
                action = self.controller.decide(knowledge)
                if action is None:
                    verbose_logger.warning(
                        f"Controller {self.verbose_name()} returned a non-action."
                    )
                    controller.ControllerExceptionReport(
                        self.verbose_name(), "a non-action returned"
                    ).log(logging.WARN)
                    return Action.DO_NOTHING
                return action
            except Exception as e:
                verbose_logger.warning(
                    f"Controller {self.verbose_name()} throw an unexpected exception: {repr(e)}."
                )
                controller.ControllerExceptionReport(self.verbose_name(), repr(e)).log(
                    logging.WARN
                )
                return Action.DO_NOTHING
        else:
            verbose_logger.warning(
                f"Controller {self.verbose_name()} was non-existent."
            )
            controller.ControllerExceptionReport(
                self.verbose_name(), "controller non-existent"
            ).log(logging.WARN)
            return Action.DO_NOTHING
    ...
    def turn_left(self) -> None:
        ...
        verbose_logger.debug(
            f"Champion {self.controller.name} is now facing {self.facing}."
        )
    ...
    def attack(self) -> None:
        ...
        verbose_logger.debug(
            f"Champion {self.controller.name} attacked with its {self.weapon.description().name}."
        )
        ChampionAttackReport(self.controller.name, self.weapon.description().name).log(
            logging.DEBUG
        )
    ...
    def damage(self, wounds: int) -> None:
        ...
        verbose_logger.debug(
            f"Champion {self.controller.name} took {wounds} wounds, it has now {self.health} hp left."
        )
        ChampionWoundsReport(self.controller.name, wounds, self.health).log(
            logging.DEBUG
        )
        if not self.alive:
            self.die()
    ...
    def die(self) -> None:
        ...
        self.arena.terrain[self.position].loot = (
            self.weapon if self.weapon.droppable() else None
        )
